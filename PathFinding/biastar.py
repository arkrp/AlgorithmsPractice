""" #  
Contains the Bidirectional AStar pathfinder.

This pathfinder is a tunable general purpose pathfinding algorithm.

This algorithm is more complex than pure A-Star but tends to outperform it in conditions where the traversal graph has a large branching factor!

classes
BiAStar - a pathfinding.Pathfinder which allows for general purpose pathfinding.
"""
# 
#  import stuff
from .pathfinder import ShortestBidirectionalPathfinder, Hueristic, NullHueristic, NoPathError
from .graphobjects import Node, Path, Edge
from ..DataStructures.heap import Heap
from dataclasses import dataclass, field
# 
class BiAStar(ShortestPathfinder): #  
    """ #  
    Wrapper for Bidirectional A-Star pathfinding algorithm
    ...
    Bidirectional A Star is a further modification of the A-Star algorithm to increase performance in cases where both the start and ending nodes are known

    If no hueristic is provided, this algorithm is equivilant to Bidirectional Dijkstra's algorithm.
    ...
    # 
    @staticmethod #  
    def find_path(starting_nodes: [Node], ending_nodes: [Node], *, hueristic_side_1: Hueristic=NullHueristic, hueristic_side_2: Hueristic=NullHueristic) -> Path:
        """ #  
        Bidirectioal A-Star pathfinding algorithm
